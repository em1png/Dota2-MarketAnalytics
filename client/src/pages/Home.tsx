...
const Home = () => {
  ...
  const itemsCostSteamByWeeks = items && items.reduce((acc: number[], value) => {
    for(let i = 0; i < value.steamData.byWeeksData.length; i++) {
      acc[i] = (acc[i] || 0) + value.steamData.byWeeksData[i][1];
    }
    return acc;
  }, []);
  ...
  const lastUpdateDate = new Date(2023, 11, 22);
  const daysFromLastUpdate = Math.round((currentDate.getTime() - lastUpdateDate.getTime()) / (24 * 60 * 60 * 1000));

  const itemsCostSteamToday = items && items.reduce((acc, value) => acc + value.price.steam, 0)
  const itemsCostD2marketToday = items && items.reduce((acc, value) => acc + value.price.d2market, 0)

  return (
    <main className="container mb-10 overflow-hidden">
      <section className="flex-start gap-3 noborder-frame">
        <img src="/logo.png" alt="logo" height={60} width={60} className="object-contain h-[60px]" />
        <div className="flex flex-col">
          <h1 className="text-3xl font-bold">EM — ONE</h1>
          <h2 className="font-medium text-neutral-400">Инструмент для анализа торговой площадки Dota 2!</h2>
        </div>
      </section>

      <section className="flex flex-col xl:flex-row xl:gap-5">
        <StatFrame className='border-frame flex-1' title={'⌬ Последнее обновление'} value={new Date(2023, 12, 22).toLocaleDateString()} desc={`${daysFromLastUpdate} дн(я)ей прошло дней с момента последнего обновления`} />
      ...
      <section className="flex flex-col xl:flex-row gap-10">
        <div className="border-frame flex-1 bg-gradient-to-r to-70% from-green-600/5">
          <div className="border-b border-neutral-800 pb-5 mb-5">
            ...
            <p className="text-xs text-neutral-400">Соотношение текущей цены к средней цене за год</p>
          </div>
          <ul className="flex flex-col gap-2 mb-5">
            {items ? [...items].sort((a, b) => (b.steamData.averagePriceYear / (b.price.steam / 100)) - (a.steamData.averagePriceYear / (a.price.steam / 100))).slice(0, 5).map((item, index) => (
              <li key={item._id} className="flex-start gap-3 pb-3 border-b last:border-b-0 border-neutral-900">
                <div className="flex-center min-w-[35px] min-h-full rounded-md text-center">
                  <span className="text-xl text-neutral-400">{index + 1}</span>
                </div>
                ...
                <Link to={`/item/${item._id}`} className="flex gap-3">
                  ...
                  <div className="flex-col-start min-w-max">
                    <span className="text-sm text-neutral-400">{item.heroName}</span>
                    <span className="font-semibold">{item.itemName}</span>
                  </div>
                </Link>

                <div className="flex flex-col items-end w-full self-end">
                  <span className="font-semibold text-xl">{item.price.steam} ₽</span>
                  <span className="text-sm text-green-400">{(item.steamData.averagePriceYear / (item.price.steam / 100)).toFixed(0)}% ∙ {item.steamData.averagePriceYear} ₽</span>
                </div>
              </li>
            ))
              :
        ...
        <div className="border-frame flex-1 bg-gradient-to-l to-70% from-rose-600/5">
          ...
          <ul className="flex flex-col gap-2 mb-5">
            {items ? [...items].sort((a, b) => (b.price.steam / (b.steamData.averagePriceYear / 100)) - (a.price.steam / (a.steamData.averagePriceYear / 100))).slice(0, 5).map((item, index) => (
              <li key={item._id} className="flex-start gap-3 pb-3 border-b last:border-b-0 border-neutral-900">
                <div className="flex-center min-w-[35px] min-h-full rounded-md text-center">
                  <span className="text-xl text-neutral-400">{index + 1}</span>
                </div>
                ...
                <div className="flex flex-col items-end w-full self-end">
                  <span className="font-semibold text-xl">{item.price.steam} ₽</span>
                  <span className="text-sm text-rose-400">{(item.steamData.averagePriceYear / (item.price.steam / 100) - 100).toFixed(2)}% ∙ {item.steamData.averagePriceYear} ₽</span>
                </div>
              </li>
            ))
              :
