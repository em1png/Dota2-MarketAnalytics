...
export const getAll = async (req, res) => {
  try {
    ...
    const itemPromises = existingItem.map(async (item) => {
      ...
      const { heroName, itemName, imageUrl, price: { steam } } = currentItem;
      return { ...item.toObject(), itemInfo: { heroName, itemName, imageUrl, currentPrice: steam } };
    });
...
export const directCreate = async (userId, itemName, price, quantity) => {
  try {
    ...
    logger.error("directCreate user owned item error:", err);
    return false;
  }
};